...
module.exports = {
   ...
   * 車両一覧画面に必要なデータ
   */
  findVehicleList: async function () {
    // sql
   ...
   * 車両詳細画面に必要なデータ
   */
  findVehicleDetail: async function (auctionId) {
    // sql
   ...
   * 売上管理に必要なデータ（詳細情報）
   */
  findSaleList: async function (id) {
    // sql
    const sql = `SELECT * FROM view_sales_management`;
    // select実行
    const connection = await mysql.createConnection(dbConf);
